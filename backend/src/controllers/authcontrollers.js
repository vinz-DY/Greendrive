...
const log = async (req, res, next) => {
  try {
    ...
    if (login) {
      ...
      if (passwordMatch) {
        const infoProfil = await tables.user.readProfil(login.id);
        ...
        res.status(200).json({
          ...
          profil: {
            id: infoProfil.id,
            name: infoProfil.name,
            lastname: infoProfil.lastname,
            birthdate: infoProfil.birthdate,
            image: infoProfil.image,
            user_id: infoProfil.user_id,
          },
        });
      } else {
        res.sendStatus(403);
